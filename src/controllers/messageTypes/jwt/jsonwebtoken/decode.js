...
jwtDecoder.verifyJwtSignatureUsingKeys = function(token, secretOrPublicKey, tokenProfile, otherOptions, baseEncoding, callback){
...
jwtDecoder.verifyJwtSignatureUsingJwk = function(token, secretOrPublicKey, tokenProfile, otherOptions, certs, baseEncoding, callback){
  var jwt = jws.decode(token, secretOrPublicKey, tokenProfile, otherOptions, callback);
    if (!jwt.signature) {
      return false;
    } else {
