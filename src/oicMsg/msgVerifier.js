...
class MessageVerifier {
  ...
  verifyPayload(payload, tokenProfile, otherOptions, callback) {
    ...
    const clockTimestamp = otherOptions.clockTimestamp || Math.floor(Date.now() / 1000);
    ...
    if (options.aud) {
      const audiences = Array.isArray(options.audience)? options.aud: [options.aud];
      const target = Array.isArray(payload.aud) ? payload.aud : [payload.aud];

      const match = target.some(targetAudience => audiences.some(audience => audience instanceof RegExp ? audience.test(targetAudience) : audience === targetAudience));
