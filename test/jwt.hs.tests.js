...
describe('HS256', function() {
  ...
  describe('when signing a token', function() {
    ...
    it('should be able to validate without options', function(done) {
      ...
      var decodedPayload = basicIdToken.fromJWT(token, secret, {"iss" :
    ...
    it('should throw when verifying null', function(done) {
      basicIdToken.fromJWT(
          ...
          {'clockTimestamp': clockTimestamp}, function(err, decoded) {
            assert.isUndefined(decoded);
            assert.isNotNull(err);
          });
      done();
  ...
  jwt.verify(token, secret, { algorithm: 'HS256' }, function(err, decoded) {
    ...
    it('should NOT return an error when the token is expired with "ignoreExpiration"',
       function(done) {
         ...
         /*    jwt.verify(token, secret, { algorithm: 'HS256', ignoreExpiration:
         true }, function(err, decoded) { assert.ok(decoded.foo);
           assert.equal('bar', decoded.foo);
           assert.isNull(err);
           done();
         });*/
    ...
    it('should default to HS256 algorithm when no options are passed',
       function() {
         ...
         var verifiedToken = basicIdToken.fromJWT(
             token, secret, {
               'iss': 'issuer',
               'sub': 'subject',
               'aud': 'audience',
  ...
  describe(
      ...
      function() {
        ...
        it('should return the "invalid token" error', function(done) {
          ...
          /*
          jwt.verify(malformedToken, secret, { algorithm: 'HS256',
